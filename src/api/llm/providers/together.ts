...
const MODEL = process.env.LLM_MODEL || 'meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo';
const API_KEY = process.env.LLM_API_KEY || '';
// Together oferece endpoint compatível com OpenAI
const BASE_URL = process.env.LLM_BASE_URL || 'https://api.together.xyz/v1';
...
export class TogetherProvider implements LlmProvider {
  async chat(input: LlmProviderInput): Promise<LlmProviderOutput> {
    if (!API_KEY) throw new Error('TOGETHER_API_KEY ausente (LLM_API_KEY)');
    ...
    if (!res.ok) {
      ...
      throw new Error(`Together error ${res.status}: ${text || res.statusText}`);
    }
