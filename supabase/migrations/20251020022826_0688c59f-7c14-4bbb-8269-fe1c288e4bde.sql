-- Atualizar a função process_invitation para enviar emails via supabase.functions.invoke no lado do cliente
-- A chamada HTTP será feita do código React, não do PostgreSQL
...
BEGIN
  ...
  FROM profiles
  WHERE id IN (
    SELECT id FROM auth.users WHERE email = _invitee_email
  )
  LIMIT 1;
  ...
  -- Criar registro na tabela event_invitations
